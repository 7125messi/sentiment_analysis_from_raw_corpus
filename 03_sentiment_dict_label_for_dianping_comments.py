...
# 情感词典打标签
"""
基于大众点评网评论数据的正则情感字典标签
"""
class DictClassifier:
    def __init__(self):
        self.__root_filepath = "./dict/"

        jieba.load_userdict("./dict/user_dict2.txt")  # 准备分词词典
    ...
    def analyse_sentence(self, sentence, runout_filepath=None, print_show=False):
        ...
        seg_result = [word.word for word in posseg.lcut(sentence) if word.flag[0] != 'x']
        # seg_result = [" ".join]
        ...
        #     comment_analysis["score"] = 1
        # else:
        #     comment_analysis["score"] = -1
        if comment_analysis["score"] > 0.4:
            comment_analysis["score"] = 2
        elif comment_analysis["score"] > -0.4:
            comment_analysis["score"] = 1
        else:
            comment_analysis["score"] = 0
        ...
        if runout_filepath is not None:
            # 将整句写进运行输出文件，以便复查
            # out = sentence.strip() + "\t" + str(seg_result) + "\t" + str(comment_analysis['score'])
            out = sentence.strip() + "\t" + str(comment_analysis['score'])
            self.__write_runout_file(runout_filepath, out + "\n")
    ...
    # 根据某些标点符号将一个评论分成若干，使用__split_sentence函数
...
if __name__ == "__main__":
    filepath_in = './data/corpus.txt'
    filepath_out = './data/corpus_result01.txt'
    print_show = False
    dc = DictClassifier()
    dc.analysis_file(filepath_in,filepath_out,print_show=False)